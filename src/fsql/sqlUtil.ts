...
// Wraps an identifier in double quotes, escaping any embedded quotes.
export function quoteIdentifier(value: string) {
  return `"${value.replace(/"/g, '""')}"`;
}

// Parses a "schema"."table" value into its parts.
export function parseQualifiedTable(table: string): { schema: string; table: string } | undefined {
  const match = /^"((?:[^"]|"")+)"\."((?:[^"]|"")+)"$/.exec(table);
  if (!match) {
    return undefined;
  }
  return { schema: match[1].replace(/""/g, '"'), table: match[2].replace(/""/g, '"') };
}
...
export function toRawSql({ sql, table }: SQLQuery): string {
  ...
  if (table) {
    rawQuery += `FROM ${formatTableName(table)} `;
  }

  const conditions = [];
  // Schema-qualified tables (e.g. system tables) are not time series and
  // have no time column, so the time filter only applies to measurements.
  if (!table || !parseQualifiedTable(table)) {
    // $__timeFrom and $__timeTo will be interpolated on the backend
    conditions.push('"time" >= $__timeFrom AND "time" <= $__timeTo');
  }
  if (sql.whereString) {
    ...
    const subst = `$1"$2"$3`;
    conditions.push(sql.whereString.replace(wherePattern, subst));
  }
  if (conditions.length > 0) {
    rawQuery += `WHERE ${conditions.join(' AND ')} `;
  }
...
// Quotes the table reference exactly once. Table names may contain dots, so
// values are never split. "schema"."table" values are already fully quoted.
function formatTableName(table: string): string {
  if (parseQualifiedTable(table)) {
    return table;
  }

  return quoteIdentifier(unquoteIdentifier(table));
}
...
function formatColumnName(parameter: string | undefined): string {
  ...
  return quoteIdentifier(unquoteIdentifier(parameter));
}
